...
@app.route("/")
def home():
    flash("Welcome to my website!! I've some awesome Tech-News for You!!!", "primary")
...
def contact():
    ...
    # sno, name,email,phone_num,mes,date
        ...
        db.session.commit()
        mail.send_message('New Message from Coding Masters Blog' + ' by ' + name,
                          sender=email,
                          recipients=[params['gmail-user']],
                          body = message + "\nSent from: " + phone
                          )
        flash("Your message was sent Succesfully!! We will get back to you soon...", "success")
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            date = datetime.now()
            if (sno=='0'):
                post = Posts(title=box_title, sub_title=box_sub_title, author=box_author, slug=box_slug, content=box_content, img_file=box_img_file, date=date)
                db.session.add(post)
                db.session.commit()
            else:
                post = Posts.query.filter_by(sno=sno).first()
